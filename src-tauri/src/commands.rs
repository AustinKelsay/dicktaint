//! Tauri command handlers and native dictation session orchestration.
...
pub(crate) fn start_native_dictation_inner(app: &tauri::AppHandle) -> Result<u64, String> {
...
pub(crate) async fn stop_native_dictation_inner(app: tauri::AppHandle) -> Result<String, String> {
    let recording = {
        let dictation = app.state::<DictationState>();
        let mut guard = dictation
            .active_recording
...
pub(crate) fn cancel_native_dictation_inner(app: &tauri::AppHandle) -> Result<(), String> {
    let recording = {
        let dictation = app.state::<DictationState>();
        let mut guard = dictation
            .active_recording
...
pub(crate) fn cancel_native_dictation_if_active(app: &tauri::AppHandle) -> Result<(), String> {
