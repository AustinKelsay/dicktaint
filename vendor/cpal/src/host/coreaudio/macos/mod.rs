...
where
    E: FnMut(StreamError) + Send + 'static,
{
    let stream_inner_weak: Weak<Mutex<StreamInner>> = Arc::downgrade(&stream.inner);
    ...
    stream_inner._disconnect_listener = Some(AudioObjectPropertyListener::new(
        ...
        move || {
            if let Some(stream_inner) = stream_inner_weak.upgrade() {
                let mut stream_inner = stream_inner.lock().unwrap();
                if stream_inner.playing {
                    let _ = stream_inner.audio_unit.stop();
                    stream_inner.playing = false;
                }
            }
